...
class Game:
    ...
    def __init__(self, agents):
        ...
        while len(self.spies) < Agent.spy_count[self.num_players]:
            ...
            if spy not in self.spies and self.agents[spy].name[:1] == 's':
                self.spies.append(spy)

        #Dhruv's addition
        # print("These are the spies:", end=" ")
        # print(self.spies)
        # for i in self.spies:
        #     print(self.agents[i], end=" ")
        # print()
    ...
    def __str__(self):
        ...
        didTestWin = True
        for i in self.spies:
            if str(self.agents[i]) == "Agent Test":
                isTestSpy = True
        ...
        if self.missions_lost<3:
            ...
            didTestWin = not isTestSpy
        else:
            s = s + '\nThe Resistance failed!'
            didTestWin = isTestSpy
        s = s + 'The spies were agents: '+ str(self.spies)

        s = s + f"\nTest was {'spy' if isTestSpy else 'resistance'} and it {'won' if didTestWin else 'lost'}"
        s = s + f"\nmissions lost: {self.missions_lost}"
        return ("won\n" if self.missions_lost<3 else "lost\n")  + s
